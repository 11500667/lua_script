...
local function whdjmhtj(self, bureau_id,resource_id_int)
	...
	ngx.log(ngx.ERR, "[djmh] -> 调用多级门户统计维护接口：bureau_id: [", bureau_id, "], resource_id_int: [", resource_id_int, "]");
	...
	local before_get = ssdb_db: get("djmh_rescount_"..bureau_id)[1];
	ngx.log(ngx.ERR, "[djmh] -> before_get: [", before_get, "]");
	
	--将bureau_id下的资源总数-1
	ssdb_db:decr("djmh_rescount_"..bureau_id)	
	
	local after_get = ssdb_db: get("djmh_rescount_"..bureau_id)[1];
	ngx.log(ngx.ERR, "[djmh] -> after_get: [", after_get, "]");
