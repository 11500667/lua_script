...
local sql = mysql_db:query("select t1.stage_id,t1.subject_id,CONCAT(CASE t1.STAGE_ID WHEN 4 THEN '小学' WHEN 5 THEN '初中' ELSE '高中' END,t2.SUBJECT_NAME) AS subject_name,COUNT(1)+(SELECT COUNT(1) FROM t_sjk_paper_info where SUBJECT_ID=t1.subject_id and GROUP_ID = "..bureau_id.." and B_DELETE=0) AS count from t_resource_info t1 inner join t_dm_subject t2 on t1.SUBJECT_ID=t2.SUBJECT_ID where t1.GROUP_ID="..bureau_id.." AND RES_TYPE IN (1,2,4,5) AND RELEASE_STATUS IN (1,3) AND t1.ts>2015042218254400372 GROUP BY SUBJECT_ID ORDER BY COUNT(1) DESC LIMIT "..show_size..";")
