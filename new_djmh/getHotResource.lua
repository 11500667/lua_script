...
local res_new_tab = {}
if res_type == "1" then	
	local res_sql = mysql_db:query("SELECT SQL_NO_CACHE id FROM t_resource_info_sphinxse WHERE query='"..group_str..stage_str.."filter=res_type,1,13;filter=release_status,1,3;groupby=attr:resource_id_int;groupsort=down_count desc;limit="..show_size.."';")
	for i=1,#res_sql do
		...
		ngx.log(ngx.ERR,"@@@"..tostring(res_info[24]).."@@@")
		--根据版本ID获取该版本属于哪个学科    
...
elseif res_type == "2" then
	local res_sql = mysql_db:query("SELECT SQL_NO_CACHE id FROM t_resource_info_sphinxse WHERE query='"..group_str..stage_str.."filter=res_type,2;filter=release_status,1,3;!filter=bk_type,1;groupby=attr:resource_id_int;groupsort=down_count desc;limit="..show_size.."';")
	for i=1,#res_sql do
		--local res_info = cache:hmget("resource_"..res_sql[i]["id"],"resource_title","resource_format","resource_page","file_id","thumb_id","preview_status","width","height","for_urlencoder_url","for_iso_url","down_count","scheme_id_int","resource_id_int")
		local res_info = ssdb_db:multi_hget("resource_"..res_sql[i]["id"],"resource_title","resource_format","resource_page","file_id","thumb_id","preview_status","width","height","for_urlencoder_url","for_iso_url","down_count","scheme_id_int","resource_id_int")
		--根据版本ID获取该版本属于哪个学科    
...
elseif res_type == "4" then
	local res_sql = mysql_db:query("SELECT SQL_NO_CACHE id FROM t_sjk_paper_info_sphinxse WHERE query='"..group_str..stage_str.."sort=attr_desc:down_count;limit="..show_size.."';")
	...
	local res_sql = mysql_db:query("SELECT SQL_NO_CACHE id FROM t_wkds_info_sphinxse WHERE query='"..group_str..stage_str.."filter=b_delete,0;sort=attr_desc:play_count;limit="..show_size.."';")
