...
ngx.say("SELECT SQL_NO_CACHE id FROM t_wkds_info_sphinxse  WHERE query=\'"..keyword..structure_scheme.."filter=type,"..wkds_type..";filter=type_id,"..bType..";filter=b_delete,0;select=("..str_group..") as match_qq;filter= match_qq, 1;"..sort_filed.."maxmatches="..str_maxmatches..";offset="..offset..";limit="..limit.."\';SHOW ENGINE SPHINX  STATUS;")

--]]
local wkds = ""
wkds = db:query("SELECT SQL_NO_CACHE id FROM t_wkds_info_sphinxse  WHERE query=\'"..keyword..structure_scheme.."filter=type,"..wkds_type..";filter=type_id,"..bType..";filter=b_delete,0;filter=ISDRAFT,0;select=("..str_group..") as match_qq;filter= match_qq, 1;"..sort_filed.."maxmatches="..str_maxmatches..";offset="..offset..";limit="..limit.."\';SHOW ENGINE SPHINX  STATUS;")
           ...
           for j=1,#wkds_value do
                 ...
                 if j==12 then 
                    ...
                    local content_json = wkds_value[17]
                     ...
                     local thumb_id = "E6648460-4FFD-E626-1C14-6FBF9F94A23C"

                     if #data.sp_list~=0 then

                        local resource_info_id = data.sp_list[1].id
                        if resource_info_id ~= ngx.null then
                          thumbid = cache:hmget("resource_"..resource_info_id,"thumb_id")
                          str = string.gsub(str,"##",thumbid[1],1)
                        end                              
                     else
                       str = string.gsub(str,"##",thumb_id)
                     end
                 else  
...
--ngx.say("{\"success\":\"true\",\"totalRow\":\""..totalRow.."\",\"totalPage\":\""..totalPage.."\",\"pageNumber\":\""..pageNumber.."\",\"pageSize\":\""..pageSize.."\",\"list\":["..wkds_info.."]}")
