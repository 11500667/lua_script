...
for i=1,#stage_list,2 do
    ...
    local arr_filesize = {}
    local filesize = {}
    local res = ngx.location.capture("/dsideal_yy/ypt/region/getDistrictByCity?city_id="..shi_id)
    local str =  cjson.decode(res.body)
    qu_list = str.table_list
    for j=1,#qu_list do
        --区名和各科的文件个数数组
          local filecount_tab = {}
          --区名和各科的文件大小数组
        local filesize_tab = {}
        local qu_id = qu_list[j].district_id
        local qu_name = qu_list[j].district_name
        filecount_tab[1] = qu_name
        filesize_tab[1] = qu_name
        for k=1,#subject_list do
            --获取学科ID
            local subject_id = subject_list[k]

            --获取个数
            local count = ssdb_db:hget("tj_shi_"..sys[n].."_"..shi_id.."_"..qu_id.."_"..stage_id.."_"..subject_id,"resource_count")[1]
            if count == "" then
            count = "0"
        end
        ...
        --获取大小
        local size = ssdb_db:hget("tj_shi_"..sys[n].."_"..shi_id.."_"..qu_id.."_"..stage_id.."_"..subject_id,"resource_size")[1]      
        if size == "" then
            size = "0"
        end
        filesize_tab[k+1] = size
          ...
          --获取个数的合计
          local total_count = ssdb_db:hget("tj_shi_"..sys[n].."_"..shi_id.."_"..qu_id.."_"..stage_id,"resource_count")[1]
          if total_count == "" then
              total_count = "0"
          end
          filecount_tab[#subject_list+2] = total_count
          --获取大小的合计
          local total_size = ssdb_db:hget("tj_shi_"..sys[n].."_"..shi_id.."_"..qu_id.."_"..stage_id,"resource_size")[1]
          if total_size == "" then
              total_size = "0"
          end 
