...
--ngx.say("SELECT SQL_NO_CACHE id FROM t_tk_question_my_info_sphinxse WHERE query=\'"..keyword..structure_scheme..str_ndid..str_qtype..bType_str..person_str..str_group.."filter=b_delete,0;groupsort=ts desc;groupby=attr:question_id_char;maxmatches="..str_maxmatches..";offset="..offset..";limit="..limit.."\';SHOW ENGINE SPHINX  STATUS;")
--ngx.say("SELECT SQL_NO_CACHE id FROM t_tk_question_my_info_sphinxse WHERE query=\'"..keyword..structure_scheme..str_ndid..str_qtype..bType_str..person_str..str_group.."sort=attr_desc:ts;maxmatches="..str_maxmatches..";offset="..offset..";limit="..limit.."\';SHOW ENGINE SPHINX  STATUS;")
...
if bType == "7" then
    ...
    for index = 1, #res do
        ...
        table.insert(objIdTable, question_id_char);
    end
...
for i=1,#res do
    ...
    if question_json[1]~=ngx.null then
        ...
        local infoSql = "select check_status from t_tk_question_info where question_id_char="..ngx.quote_sql_str(jsonQues.question_id_char).." and group_id=2 and check_status!=0";


        local resultInfo = db:query(infoSql);

        if resultInfo and #resultInfo >0 then
            if resultInfo[1]["check_status"]==1 then
                chkDsidealStr = "全国-审核通过";
            elseif resultInfo[1]["check_status"]==2 then
                chkDsidealStr = "全国-待审核";
            elseif resultInfo[1]["check_status"]==3 then
