...
从/usr/local/lua_script/wkds/wkds_info_list.lua拷贝，改成不登录即可访问
@Author  feiliming
@Date    2015-5-11
]]
...
--]]
ngx.log(ngx.ERR,"@@@@@@@@@@@@@@@@@@@@@SELECT SQL_NO_CACHE id FROM t_wkds_info_sphinxse  WHERE query=\'"..keyword..structure_scheme.."filter=type,"..wkds_type..";"..str_group..person_str..bType_str..sort_filed.."filter=isdraft,0;filter=check_status,0,1;filter=b_delete,0;maxmatches="..str_maxmatches..";offset="..offset..";limit="..limit.."\';SHOW ENGINE SPHINX  STATUS;@@@@@@@@@@@@@@@@@@@@@")
local wkds = ""
wkds = db:query("SELECT SQL_NO_CACHE id FROM t_wkds_info_sphinxse  WHERE query=\'"..keyword..structure_scheme.."filter=type,"..wkds_type..";"..str_group..person_str..bType_str..wk_type_str..sort_filed.."filter=isdraft,0;filter=check_status,0,1;filter=b_delete,0;maxmatches="..str_maxmatches..";offset="..offset..";limit="..limit.."\';SHOW ENGINE SPHINX  STATUS;")
...
for i=1,#wkds do
      ...
      local str = "{\"id\":\""..wkds[i]["id"].."\",\"wkds_id_int\":\"##\",\"wkds_id_char\":\"##\",\"scheme_id_int\":\"##\",\"structure_id\":\"##\",\"wkds_name\":\"##\",\"study_instr\":\"##\",\"teacher_name\":\"##\",\"play_count\":\"##\",\"score_average\":\"##\",\"create_time\":\"##\",\"download_count\":\"##\",\"thumb_id\":\"##\",\"downloadable\":\"##\",\"person_id\":\"##\",\"table_pk\":\"##\",\"group_id\":\"##\",\"content_json\":\"##\",\"wk_type\":\"##\",\"wk_type_name\":\"##\",\"type_id\":\"##\",\"uploader_id\":\"##\",\"stage_id\":\"##\",\"subject_id\":\"##\",\"parent_structure_name\":\"##\",\"person_name\":\"##\",\"org_name\":\"##\"}"

      local wkds_value_null = cache:hmget("wkds_"..wkds[i]["id"],"wkds_id_int")
    if wkds_value_null[1] ~=ngx.null then

       local wkds_value = cache:hmget("wkds_"..wkds[i]["id"],"wkds_id_int","wkds_id_char","scheme_id","structure_id","wkds_name","study_instr","teacher_name","play_count","score_average","create_time","download_count","thumb_id","downloadable","person_id","table_pk","group_id","content_json","wk_type","wk_type_name","type_id","uploader_id","stage_id","subject_id")
         local  thumb_id = ""
