...
从/usr/local/lua_script/new_resource_my_list.lua拷贝，改成不登录即可访问
@Author  feiliming
@Date    2015-5-11
]]
...
ngx.log(ngx.ERR,"SELECT SQL_NO_CACHE id FROM t_resource_my_info_sphinxse WHERE query=\'"..keyword..structure_scheme.."filter=b_delete,0;filter=res_type,"..res_type..";"..str_rtype..sort_filed..bType_str..person_str..str_group..str_app.."maxmatches="..str_maxmatches..";offset="..offset..";limit="..limit.."\';SHOW ENGINE SPHINX  STATUS;");
local res = db:query("SELECT SQL_NO_CACHE id FROM t_resource_my_info_sphinxse WHERE query=\'"..keyword..structure_scheme.."filter=b_delete,0;filter=res_type,"..res_type..";"..str_rtype..sort_filed..bType_str..person_str..str_group..str_app.."maxmatches="..str_maxmatches..";offset="..offset..";limit="..limit.."\';SHOW ENGINE SPHINX  STATUS;")
...
if bType == "7" then
    ...
    for index = 1, #res do
        ...
        local resIdInt = ssdb_db:multi_hget("myresource_" .. myInfoId, "resource_id_int");

        table.insert(objIdTable, resIdInt[2]);
    end
...
for i=1,#res do
    ...
    if tostring(resource_value_null[2]) ~= "userdata: NULL" then
        ...
        if chkStatusTable ~= nil then
            local chkStatusStr = chkStatusTable[resource_value[1]];
