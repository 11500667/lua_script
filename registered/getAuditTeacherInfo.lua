...
if args["org_id"] == nil or args["org_id"] == "" then
	...
	sqlwhere = sqlwhere.." AND org_id='"..org_id.."' "
end
...
local res_count = db:query("SELECT count(1) as count FROM t_base_person "..sqlwhere.. ";")
...
local students, err, errno, sqlstate = db:query("SELECT person_id,person_name,xb_name,org_id,tel,b_use FROM t_base_person "..sqlwhere.." order by person_id DESC LIMIT "..offset..","..limit..";")
ngx.log(ngx.ERR,"SELECT person_id,person_name,xb_name,org_id,tel FROM t_base_person WHERE "..sqlwhere.." order by person_id DESC LIMIT "..offset..","..limit..";")
...
for i=1,#students do
  local ssdb_info = {};
  ssdb_info["person_id"] = students[i]["person_id"];
  ssdb_info["person_name"]= students[i]["person_name"];
  ssdb_info["sex"] = students[i]["xb_name"];
  ssdb_info["department"]= students[i]["org_id"];
  ssdb_info["tel"] = students[i]["stu_tel"];
