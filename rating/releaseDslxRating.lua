...
if tostring(exist_res[1]["count"]) == "0" then
	...
	local querysql = "select tbp.org_id,tbo.org_name from t_dswk_login tbp ,t_base_organization tbo  where tbp.org_id=tbo.ORG_ID and person_id='"..person_id.."' union all select tdl.org_id,tdo.org_name from t_dswk_login tdl,t_dswk_organization tdo where tdl.ORG_ID=tdo.ORG_ID and  person_id='"..person_id.."'"
	ngx.log(ngx.ERR,querysql)
	local querysql_res,err,errno,sqlstatus = mysql_db:query(querysql)
	if not querysql_res then
		ngx.say("{\"success\":\"false\",\"info\":\"查询过程出错！\"}");
		ngx.log(ngx.ERR, "err: ".. err);
	end
	local bureau_id = querysql_res[1]["org_id"]
	local bureau_name = querysql_res[1]["org_name"]
