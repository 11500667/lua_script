...
--奖项ID      1：一等  2：二等  3：三等  4：优胜
if args["award_id"] == nil or args["award_id"] == "" then
    ngx.say("{\"success\":\"false\",\"info\":\"award_id参数错误！\"}")
    return
end
...
if award_id ~= "-1" then
	award_str = " AND award_id="..award_id
end
...
local resource_count = mysql_db:query("SELECT count(1) as count FROM t_rating_resource WHERE rating_id = "..rating_id.." AND resource_status = 3"..stage_str..subject_str..award_str..w_type)
ngx.log(ngx.ERR, "SELECT count(1) as count FROM t_rating_resource WHERE rating_id = "..rating_id.." AND resource_status = 3"..stage_str..subject_str..award_str..w_type)
local offset = pageSize*pageNumber-pageSize
local limit = pageSize
local totalRow = resource_count[1]["count"]
local totalPage = math.floor((totalRow+pageSize-1)/pageSize)

local resource_info = mysql_db:query("SELECT id,resource_info_id,person_name,bureau_name,(SELECT rating_title FROM  t_rating_info T2 WHERE T2.id=T1.rating_id) AS rating_title,CASE AWARD_ID WHEN 1 THEN '一等奖' WHEN 2 THEN '二等奖' WHEN 3 THEN '三等奖' ELSE '优胜奖' END AS award,view_count,vote_count FROM t_rating_resource T1 WHERE rating_id = "..rating_id.." AND resource_status = 3"..stage_str..subject_str..award_str..w_type.." ORDER BY award_id ASC,view_count,vote_count DESC LIMIT "..offset..","..limit..";")
