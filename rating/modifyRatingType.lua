...
if count > 0 then
  ...
  result["info"] = "该评比类型已存在，不能修改！" 
 else
	--插入评比类型到数据库
	local update_sql="update t_rating_type set rating_type_name='"..rating_type_name.."' where rating_type_id="..rating_type_id
