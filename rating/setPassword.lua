...
--old_pwd
if args["old_pwd"] == nil or args["old_pwd"] == "" then
  ngx.say("{\"success\":false,\"info\":\"old_pwd参数错误！\"}")
  return
end
local old_pwd = ngx.md5(tostring(args["old_pwd"]))

--new_pwd
if args["new_pwd"] == nil or args["new_pwd"] == "" then
  ngx.say("{\"success\":false,\"info\":\"new_pwd参数错误！\"}")
  return
end
local new_pwd = ngx.md5(tostring(args["new_pwd"]))
...
local countsql = "select count(*) as count from t_dswk_login where person_id='"..person_id.."' and login_password='"..old_pwd.."'"
...
if tonumber(countsql_res[1]["count"]) == 0 then 
	ngx.say("{\"success\":false,\"info\":\"密码不正确\"}");
	return
end
local updatesql = "update t_dswk_login set login_password = '"..new_pwd.."' "..updatesql.." where person_id='"..person_id.."'"
