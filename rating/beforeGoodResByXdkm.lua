...
local award_str = " AND award_id IN (1,2,3,4)"
...
if rating_type ==1 then
  ...
  local resource_count = mysql_db:query("SELECT count(1) as count FROM t_rating_resource T1 WHERE resource_status =3 "..award_str.." AND rating_id IN (SELECT id FROM t_rating_info WHERE org_id = "..district_id.." AND rating_status = 4)"..stage_str..subject_str)
  
  
  local offset = pageSize*pageNumber-pageSize
  local limit = pageSize
  totalRow = resource_count[1]["count"]
  totalPage = math.floor((totalRow+pageSize-1)/pageSize)
  resource_info = mysql_db:query("SELECT id,resource_info_id,person_name,bureau_name,(SELECT rating_title FROM  t_rating_info T2 WHERE T2.id=T1.rating_id) AS rating_title,CASE AWARD_ID WHEN 1 THEN '一等奖' WHEN 2 THEN '二等奖' WHEN 3 THEN '三等奖' ELSE '优胜奖' END AS award FROM t_rating_resource T1 WHERE resource_status =3 "..award_str.." AND rating_id IN (SELECT id FROM t_rating_info WHERE org_id = "..district_id.." AND rating_status = 4)"..stage_str..subject_str.." ORDER BY AWARD_ID ASC,VOTE_COUNT DESC LIMIT "..offset..","..limit..";")
  ...
  local resource_count = mysql_db:query("SELECT count(1) as count FROM t_rating_resource T1 WHERE resource_status =3 "..award_str.." AND rating_id IN (SELECT id FROM t_rating_info WHERE rating_status = 4 and rating_type="..rating_type..")"..stage_str..subject_str..w_type)
  
  ngx.log(ngx.ERR, "SELECT count(1) as count FROM t_rating_resource T1 WHERE resource_status =3 "..award_str.." AND rating_id IN (SELECT id FROM t_rating_info WHERE rating_status = 4 and rating_type="..rating_type..")"..stage_str..subject_str..w_type)
  local offset = pageSize*pageNumber-pageSize
  local limit = pageSize
  totalRow = resource_count[1]["count"]
  totalPage = math.floor((totalRow+pageSize-1)/pageSize)
	resource_info = mysql_db:query("SELECT id,resource_info_id,person_name,bureau_name,(SELECT rating_title FROM  t_rating_info T2 WHERE T2.id=T1.rating_id) AS rating_title,CASE AWARD_ID WHEN 1 THEN '一等奖' WHEN 2 THEN '二等奖' WHEN 3 THEN '三等奖' ELSE '优胜奖' END AS award FROM t_rating_resource T1 WHERE resource_status =3 "..award_str.." AND rating_id IN (SELECT id FROM t_rating_info WHERE rating_status = 4 and rating_type="..rating_type..")"..stage_str..subject_str..w_type.." ORDER BY AWARD_ID ASC,VOTE_COUNT DESC LIMIT "..offset..","..limit..";")
