...
--获取参数person_id，并判断参数是否正确
if args["old_person"] == nil or args["old_person"] == "" then
    ngx.say("{\"success\":false,\"info\":\"old_person_id参数错误！\"}")
    return
end

local old_person_id = args["old_person"];
...
local ssdb_key_old_person="school_admin_person_"..old_person_id;

local res, err = ssdb_db:del(ssdb_key_old_person);
if not res then 
    ngx.say("{\"success\":false,\"info\":\""..err.."\"}")
    return
end
...
local res, err = ssdb_db:set(ssdb_key,person_id);
if not res then 
    ngx.say("{\"success\":false,\"info\":\""..err.."\"}")
    return
end

local res, err = ssdb_db:set(ssdb_key_person,1);
if not res then 
    ngx.say("{\"success\":false,\"info\":\""..err.."\"}")
    return
end
