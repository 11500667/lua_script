...
if args["subject_name"] == nil or args["subject_name"] == "" then
	...
	ngx.log(ngx.ERR, "ERR MSG =====> 参数subject_name不能为空！");
    return
end
...
if args["stage_name"] == nil or args["stage_name"] == "" then
	...
	ngx.log(ngx.ERR, "ERR MSG =====> 参数stage_name不能为空！");
    return
end
...
--插入学科到数据库
local insert_sql="insert into t_dm_subject(subject_name,stage_id) values ('"..subject_name.."',"..stage_id..");"
...
local subject_id = res.insert_id
...
--ngx.log(ngx.ERR,"\"xd_subject_list\":"..cjson.encode(stage_tab))
...
--放入ssdb开始
		...
		local ssdb_res ={}
		ssdb_res.stage_subject=stage_name..subject_name
		ssdb_res.stage_id=stage_id
		ssdb_res.stage_name=stage_name
		ssdb_res.subject_id=subject_id
		ssdb_res.subject_name=subject_name
	
		ssdb_db:multi_hset("subject_"..subject_id,ssdb_res);
	--[[
		local result_subject = ssdb_db:multi_hget("subject_"..subject_id,"subject_id");
		ngx.log(ngx.ERR,"hu_log============="..#result_subject)
		ngx.log(ngx.ERR,"hu_log"..result_subject[2]);
]]	
--放入ssdb结束
