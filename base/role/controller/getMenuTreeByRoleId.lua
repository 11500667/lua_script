...
local query_checked_menu_sql="select ROLE_ID,MENU_ID from  t_sys_role_menu where role_id="..role_id;
ngx.log(ngx.ERR,"hy_log-------->查询角色菜单"..query_checked_menu_sql);

local query_checked_menu_res=mysql_db:query(query_checked_menu_sql);
...
for i=1,#query_res do
	...
	menu_res["checked"]=false;
	for j=1,#query_checked_menu_res do
		if query_res[i]["MENU_ID"] ==query_checked_menu_res[j]["MENU_ID"] then
