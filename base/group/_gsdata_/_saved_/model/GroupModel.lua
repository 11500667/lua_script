...
local function queryGroup(keyWord , personId, identityId, pageNumber, pageSize)
	...
	local sql = "select ID,GROUP_NAME,GROUP_TYPE,PLAT_TYPE ,PLAT_ID,GROUP_DESC,GROUP_NOTICE,B_USE,CREATE_TIME,GROUP_LEVEL,LEVEL_ORG_ID,B_REQUEST,AVATER_URL from t_base_Group_new where B_USE=1 and GROUP_TYPE=2 ";
	
	local whereSql = "";
	
	local queryCount = "select count(1) as count from t_base_Group_new where B_USE=1 and GROUP_TYPE=2 ";
...
local function saveGroup(paramTable)
	...
	local insertGroupSql = "INSERT INTO T_BASE_GROUP_NEW (ID,PARENT_TYPE,PARENT_ID,GROUP_NAME,GROUP_TYPE,CREATOR_ID,MASTER_ID,USE_RANGE,PLAT_TYPE,PLAT_ID,GROUP_DESC,CREATE_TIME,GROUP_LEVEL,LEVEL_ORG_ID,AVATER_URL,TS) VALUES (" .. newId .. ", " .. parentType .. ", " .. parentId .. ", " .. quote(groupName) .. ", " .. groupType .. ", " .. creatorId .. ", " .. masterId .. ", " .. useRange .. ", " .. platType .. ", " .. platId .. ", " .. quote(groupDesc) .. ", " ..  quote(createTime)  .. ", " .. groupLevel .. ", " .. levelOrgId ..", " .. quote(avaterUrl)	..", " .. currentTS .. ") ";
...
local function chkGruopName(creatorId, groupName)
	...
	local queryCount = "select count(1) as count from T_BASE_Group_NEW a,T_BASE_ORGANIZATION b where a.B_USE=1 and a.CREATOR_ID = "..tonumber(creatorId).." and a.GROUP_NAME = "..quote(groupName) .." or b.ORG_NAME = "..quote(groupName);
