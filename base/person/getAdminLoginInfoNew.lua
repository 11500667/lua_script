#lzy 2015-3-15 唐山开平统一账号登录(统一认证对接使用)
...
local ssdb_value = ssdb_db:exists("school_admin_person_"..person_id.."_5");
--ngx.log(ngx.ERR,"**********************************"..#ssdb_value[1].."&&&&&&&&&&&&&&&&&"..ssdb_value[1])
if ssdb_value[1] =="1" then
local old_school_id = ssdb_db:get("school_admin_person_"..person_id.."_5");
-- 查询校管理员的人员id
local  sel_new_person_id = "SELECT PERSON_ID FROM t_base_person WHERE bureau_id ="..old_school_id[1].." and identity_id =4";
ngx.log(ngx.ERR,"================="..sel_new_person_id)
...
local login_name_sql =  "SELECT login_name FROM t_sys_loginperson WHERE person_id = "..new_person_id.." and identity_id = 4";
...
local org_info,err = cache:hmget("person_"..admin_info[1].."_4","shi","qu","xiao","sheng");
  if not org_info then
     ngx.say("{\"success\":false,\"info\":\""..err.."\"}")
     return
end
local role_str = "";
local role_info = cache:lrange("role_"..admin_info[1].."_4",0,-1)
    if #role_info ~= 0 then
...
returnJson["is_admin"] =1;	
else
returnJson["is_admin"] =0;
end
