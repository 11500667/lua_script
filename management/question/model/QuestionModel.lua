	...
	for s=1,#question_id_table do
		...
		if tonumber(check_type) == 1 then
			...
			for i=1, #res do
				...
				local ssdbKey  = "1_2_"..question_id_char.."_"..structure_id_int;
				local hashKey = "is_struc_repeat";
				local result, err = ssdb:hset(ssdbKey, hashKey,question_tem_id);

				local content_md5_new_unique_table = ssdb:hget(create_person_tem.."_"..question_id_char,"content_md5_new_unique");

				local content_md5_new_unique = content_md5_new_unique_table[1]

				local result, err = ssdb:hset("new_md5_ques_"..content_md5_new_unique, "1_2",question_id_char);
...
local function delQuestion(question_id_char,structure_ids_table)
	...
	for i=1,#structure_ids_table do
		structureStr = structureStr..structure_ids_table[i]..",";
	end
	...
	local sql = "update  t_tk_question_info set b_delete=1 , update_ts="..currentTS.." where question_id_char ="..quote(question_id_char).." and structure_id_int in ("..structureStr..") and group_id=1 and create_person=1";
	...
	if #structure_ids_table==1 then
		local ssdbKey  = "1_2_"..question_id_char.."_"..structure_ids_table[1];
		local hashList = "is_struc_repeat";
		local result, err = ssdb:multi_hdel(ssdbKey, hashList);
	else
		local structure_ids = "";
		for i=1, #structure_ids_table do
			local ssdbKey  = "1_2_"..question_id_char.."_"..structure_ids_table[i];
