...
if not level or len(level) == 0 
	or not list or len(list) == 0 then
...
if z0 and z0[1] and z0[1] ~= "ok" then
	...
	--删除
	ssdb:multi_zdel("workroom_"..level.."_t_hot", unpack(wids))
end

--重新set
for i=1,#t_list.list do
	ssdb:zset("workroom_"..level.."_t_hot", t_list.list[i].teacher_id, t_list.list[i].order)
end
