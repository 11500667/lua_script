...
--找工作室
local t_wk, err = ssdb:hget("workroom_workrooms", workroom_id)
...
local base_sql = "SELECT IFNULL(st.STAGE_NAME,'全部')as stage_name,IFNULL(st.stage_id,0) as stage_id,teacher_count as teacher_num from t_base_workroom_tj wt left join t_dm_stage st on st.STAGE_ID = wt.stage_id where wt.workroom_id = "..workroom_id.." and wt.subject_id=0 GROUP BY stage_name,stage_id";
local base_result = db:query(base_sql)

local all_sql = "SELECT IFNULL(st.STAGE_NAME,'全部')as stage_name,IFNULL(st.stage_id,0) as stage_id,ifnull(su.SUBJECT_NAME,'全部')as subject_name,ifnull(su.subject_id,0)as subject_id,teacher_count as teacher_num from t_base_workroom_tj wt left join t_dm_stage st on st.STAGE_ID = wt.stage_id LEFT JOIN t_dm_subject su on wt.subject_id = su.subject_id where wt.workroom_id = "..workroom_id.." and teacher_count>0";
ngx.log(ngx.ERR, "cxg_log  getWorkroomDetailByWorkroomID ============> "..all_sql.."");
