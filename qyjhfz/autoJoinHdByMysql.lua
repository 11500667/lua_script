...
--获取详细信息
local t = {}
local querySql = "select xzt_id,person_id,start_time as start_date,end_time as end_date,hd_confid,con_pass from t_qyjh_hd where hd_id = "..hd_id
...
t.xzt_id = result[1]["xzt_id"]
t.person_id = result[1]["person_id"]
...
if etonum < ts then
	...
	local xzt_sql = "select xzt_id from t_qyjh_xzt_tea where b_use=1 and xzt_id="..t.xzt_id.." and tea_id="..person_id
	local result, err, errno, sqlstate = db:query(xzt_sql);
	if not result then
		ngx.say("{\"success\":false,\"info\":\"查询协作体数据失败！\"}");
		return;
	end
	...
	if #result == 0 then
		...
		--进入活动
		local pwd = t.con_pass
		if person_id == t.person_id then
			pwd = "dsidealqyjh"
		end
		--ngx.log(ngx.ERR,"*************"..pwd.."**************8")
