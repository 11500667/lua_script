...
if obj_type == "1" then
	--获取资源数据
	res = mysql_db:query("SELECT SQL_NO_CACHE id FROM t_base_publish_sphinxse WHERE query='"..perfilter.."filter=b_delete,0;!range=hd_id,0,999999;filter=obj_type,1;filter=pub_type,3;filter=xzt_id,"..path_id..";filter=qyjh_id,"..qyjh_id..";groupby=attr:obj_info_id;groupsort=ts desc;limit="..limit.."'")
	for i=1,#res do
		...
		--根据版本ID获取该版本属于哪个学科    
		local subject_name = ssdb:hget("t_resource_scheme_"..zy_tj_new[12],"subject_name")[1]
		local subject_id = ssdb:hget("t_resource_scheme_"..zy_tj_new[12],"subject_id")[1]		
		local zy_tj_new_tab = {}
		zy_tj_new_tab["iid"] = iid
		zy_tj_new_tab["resource_title"] = zy_tj_new[1]
		zy_tj_new_tab["resource_format"] = zy_tj_new[2]
		zy_tj_new_tab["resource_page"] = zy_tj_new[3]
		zy_tj_new_tab["file_id"] = zy_tj_new[4]
		zy_tj_new_tab["thumb_id"] = zy_tj_new[5]
		zy_tj_new_tab["preview_status"] = zy_tj_new[6]
		zy_tj_new_tab["width"] = zy_tj_new[7]
		zy_tj_new_tab["height"] = zy_tj_new[8]
		zy_tj_new_tab["for_urlencoder_url"] = zy_tj_new[9]
		zy_tj_new_tab["for_iso_url"] = zy_tj_new[10]
		zy_tj_new_tab["browse_count"] = zy_tj_new[11]
		zy_tj_new_tab["resource_id_int"] = zy_tj_new[13]
		zy_tj_new_tab["subject"] = subject_name
		zy_tj_new_tab["subject_id"] = subject_id
		zy_tj_new_tab["create_time"] = zy_tj_new[14]
		zy_tj_new_tab["url_code"] = encodeURI(zy_tj_new[1])
		resource_hot_tab[i] = zy_tj_new_tab    
	end
	returnjson["zy_hot"] = resource_hot_tab
elseif obj_type == "3" then
	--获取试卷数据
	res = mysql_db:query("SELECT SQL_NO_CACHE id FROM t_base_publish_paper_sphinxse WHERE query='"..perfilter.."filter=b_delete,0;!range=hd_id,0,999999;filter=obj_type,3;filter=pub_type,3;filter=xzt_id,"..path_id..";filter=qyjh_id,"..qyjh_id..";groupby=attr:obj_info_id;groupsort=ts desc;limit="..limit.."'")
	...
	for i=1,#res do
		...
		sj_tj_new_tab["preview_status"] =resource_value[1]
		sj_tj_new_tab["for_iso_url"] =resource_value[2]
		sj_tj_new_tab["for_urlencoder_url"] =resource_value[3]
		sj_tj_new_tab["file_id"] =resource_value[4]
		sj_tj_new_tab["page"] =resource_value[5]
		sj_tj_new_tab["structure_id_int"] =resource_value[6]
		sj_tj_new_tab["structure_id"] =resource_value[6]
		sj_tj_new_tab["scheme_id_int"] =resource_value[7]
		sj_tj_new_tab["scheme_id"] =resource_value[7]
		sj_tj_new_tab["create_time"] =resource_value[8]
...
elseif obj_type == "4" then
	--获取备课数据
	res = mysql_db:query("SELECT SQL_NO_CACHE id FROM t_base_publish_sphinxse WHERE query='"..perfilter.."filter=b_delete,0;!range=hd_id,0,999999;filter=obj_type,4;filter=pub_type,3;filter=xzt_id,"..path_id..";filter=qyjh_id,"..qyjh_id..";groupby=attr:obj_info_id;groupsort=ts desc;limit="..limit.."'")
	...
	for i=1,#res do
		...
		--根据版本ID获取该版本属于哪个学科    
		local subject_name = ssdb:hget("t_resource_scheme_"..bk_tj_hot[12],"subject_name")[1] 
		local subject_id = ssdb:hget("t_resource_scheme_"..bk_tj_hot[12],"subject_id")[1]		
		local bk_tj_hot_tab = {}
		bk_tj_hot_tab["iid"] = iid
		bk_tj_hot_tab["resource_title"] = bk_tj_hot[1]
		bk_tj_hot_tab["resource_format"] = bk_tj_hot[2]
		bk_tj_hot_tab["resource_page"] = bk_tj_hot[3]
		bk_tj_hot_tab["file_id"] = bk_tj_hot[4]
		bk_tj_hot_tab["thumb_id"] = bk_tj_hot[5]
		bk_tj_hot_tab["preview_status"] = bk_tj_hot[6]
		bk_tj_hot_tab["width"] = bk_tj_hot[7]
		bk_tj_hot_tab["height"] = bk_tj_hot[8]
		bk_tj_hot_tab["for_urlencoder_url"] = bk_tj_hot[9]
		bk_tj_hot_tab["for_iso_url"] = bk_tj_hot[10]
		bk_tj_hot_tab["browse_count"] = bk_tj_hot[11]
		bk_tj_hot_tab["resource_id_int"] = bk_tj_hot[13]    
		bk_tj_hot_tab["subject"] = subject_name
		bk_tj_hot_tab["subject_id"] = subject_id
		bk_tj_hot_tab["create_time"] = bk_tj_hot[14]
		bk_tj_hot_tab["resource_size_int"] = bk_tj_hot[15]
		bk_tj_hot_tab["beike_type"] = bk_tj_hot[16]
		bk_tj_hot_tab["structure_id"] = bk_tj_hot[17]
		bk_tj_hot_tab["resource_id_char"] = bk_tj_hot[18]
		bk_tj_hot_tab["url_code"] = encodeURI(bk_tj_hot[1])
		bk_tj_hot_tab["scheme_id"] =bk_tj_hot[12]
		resource_hot_tab[i] = bk_tj_hot_tab    
	end
returnjson["bk_hot"] = resource_hot_tab	
elseif obj_type == "5" then
	--获取微课数据
	res = mysql_db:query("SELECT SQL_NO_CACHE id FROM t_base_publish_wk_sphinxse WHERE query='"..perfilter.."filter=b_delete,0;!range=hd_id,0,999999;filter=obj_type,5;filter=pub_type,3;filter=xzt_id,"..path_id..";filter=qyjh_id,"..qyjh_id..";groupby=attr:obj_info_id;groupsort=ts desc;limit="..limit.."'")
