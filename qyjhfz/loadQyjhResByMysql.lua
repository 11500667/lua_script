	...
	if xzt_id == "-1" or xzt_id == "0" or not xzt_id or string.len(xzt_id)<=0 then 
		...
		if #xzt_result>0 then
			...
			for i=1,#xzt_result,1 do
				xztids = xztids..xzt_result[i]["xzt_id"]
				if i ~= #xzt_result then
					xztids = xztids..","
				end
			end
			scopePamas = "filter=xzt_id,"..xztids..";"
		else
...
if page_type == "4" or page_type == "5" then
	...
	--****资源条件****
		...
		if app_type_id ~= "0" then 
			app_type_id_sql = " filter=app_type_id," .. app_type_id .. ";";
		else 
			app_type_id_sql = "";
		end  ]]
		--==============应用类型检索开始20150702
		if app_type_id ~= "0" then 
			app_type_id_sql="select=(app_type_id%"..app_type_id..") as match_qq;!range= match_qq,1,"..app_type_id..";"
		else 
			app_type_id_sql = "";
		end
		--==============应用类型检索开始
...
if isfljs =="0" then
	...
	--ngx.log(ngx.ERR,"cxg_log ccccccc====*********>"..sphinxSql.."<*****====")
	res = mysql_db:query(sphinxSql)
	--**
else
	local sphinxSql = "SELECT SQL_NO_CACHE id FROM "..tablefilter.." WHERE query=\';".. keyword .. app_type_id_sql ..structure_scheme .. str_rtype..perfilter..pathfilter..subject_str.."filter=b_delete,0;filter=obj_type,"..obj_type..";filter=pub_type,3;filter=qyjh_id,"..qyjh_id..";"..sort_filed.."groupby=attr:obj_info_id;maxmatches="..(offset+limit)..";offset="..offset..";limit="..limit.."\';SHOW ENGINE SPHINX STATUS;"
	   ...
	   if obj_type =="5" then 
		  ...
		  if wkds_value_null[1] ~= ngx.null then
		   ...
		   if obj_type == "3" then
			...
			--根据版本ID获取该版本属于哪个学科    
			local subject_name = ssdb:hget("t_resource_scheme_"..res_info[34],"subject_name")[1]
			local subject_id = ssdb:hget("t_resource_scheme_"..res_info[34],"subject_id")[1]
			...
			if res_info[40] ~= "-1" then
				--ngx.log(ngx.ERR,"====>"..res_info[17].."*"..res_info[20].."<====")			
				local res_person = ngx.location.capture("/dsideal_yy/apptype/get_apptypename?scheme_id="..res_info[34].."&app_type_id="..res_info[40])
