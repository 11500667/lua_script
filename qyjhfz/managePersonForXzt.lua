...
--ngx.log(ngx.ERR, "+++++++++++++++++++++++>"..xztids.."<=+++++++++++++++=");
if not xztids or string.len(xztids) == 0 then
	xztids =","
end
--获取当前用户所属的协作体结束
--获取当前教师所属大学区下所属的协作体开始
local hdxztids = ssdb:hget("qyjh_tea_xzts_"..dxq_id,person_id)
...
if operationtype == "1" then
	...
	--****************
	local id = cache:incr("t_qyjh_xzt_tea_pk")
	local insertSql = "insert into t_qyjh_xzt_tea (id,qyjh_id,dxq_id,xzt_id,org_id,tea_id,start_time)values("
			..id..","..quote(qyjh_id)..","..quote(dxq_id)..","..quote(xzt_id)..","..quote(org_id)..","..quote(person_id)..","..quote(os.date("%Y%m%d%H%M%S"))..")";
local ok, err = mysql_db:query(insertSql)
...
elseif operationtype == "2" then
	...
	local ok, err = mysql_db:query(updateSql)
end
...
ssdb:hset("qyjh_tea_xzts_"..dxq_id,person_id,dxztids)
...
if #res <= 2 then
	ssdb:hset("qyjh_xzt_tj",xzt_id.."_js_tj",0)
else
	ssdb:hset("qyjh_xzt_tj",xzt_id.."_js_tj",#res-2)	
end
--修改协作体统计中的教师数量结束
