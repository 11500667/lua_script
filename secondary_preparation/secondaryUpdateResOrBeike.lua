...
local resource_title  = tostring(args["resource_title"]);
...
--拼接sql语句
local up_base = "update t_resource_base set resource_title = '"..resource_title.."',resource_type="..resource_type..",resource_type_name='"..resource_type_name.."',extension = '"..extension.."',create_time = '"..create_time.."',resource_size_int = -1,file_id = '"..file_id.."',FILE_MD5=-1,FILE_SHA1=-1,FOR_URLEncoder_Url=-1,FOR_ISO_Url=-1,PREVIEW_STATUS="..preview_status..",THUMB_STATUS="..thumb_status..",THUMB_ID='"..thumbid.."',TS="..ts.." where resource_id_int = "..resource_id_int;  	
...
local up_info = "update t_resource_info  set resource_title = '"..resource_title.."',resource_type="..resource_type..",resource_type_name='"..resource_type_name.."',resource_format='"..extension.."',create_time = '"..create_time.."',resource_size_int = -1,file_id = '"..file_id.."',FOR_URLEncoder_Url=-1,FOR_ISO_Url=-1,PREVIEW_STATUS="..preview_status..",THUMB_STATUS="..thumb_status..",THUMB_ID='"..thumbid.."',TS="..ts..",update_ts="..update_ts.." where id = ";
...
for i=1,#info_list do
  up_info = up_info..info_list[i]["ID"];   
    ...
    local result_info, err, errno, sqlstate = db:query(up_info)
	 ...
	 --修改缓存
               resource_map.id = info_list[i]["ID"];
...
for i=1,#my_info_list do
   up_my_info = up_my_info..my_info_list[i]["ID"];   
    ...
    local result_my_info, err, errno, sqlstate = db:query(up_my_info)
	 ...
	 --修改缓存
	 resource_map.id = my_info_list[i]["ID"];
	 local resourceUtil  = require "base.resource.model.ResourceUtil";
     local result = resourceUtil:setResourceMyInfo(resource_map)
     if result~=true then
         ngx.say("{\"success\":false,\"info\":\"操作失败\"}")
     end
