...
--获取大学区统计信息开始
local xzt_tj = ssdb:hget("qyjh_dxq_tj_"..dxq_id,"xzt_tj")
local xx_tj = ssdb:hget("qyjh_dxq_tj_"..dxq_id,"xx_tj")
local js_tj = ssdb:hget("qyjh_dxq_tj_"..dxq_id,"js_tj")
local hd_tj = ssdb:hget("qyjh_dxq_tj_"..dxq_id,"hd_tj")
local zy_tj = ssdb:hget("qyjh_dxq_tj_"..dxq_id,"zy_tj")
--获取大学区统计信息结束
--根据大学区ID获取按照点击量排行的前6个协作体开始
local txztids = ssdb:zrrange("qyjh_dxq_xzt_djl_"..dxq_id,0,6)
local xztids = {}
local xzts = {}
if #txztids>=2 then
...
--根据大学区ID获取大学区信息开始
local dxqinfo
local res_org = ngx.location.capture("/dsideal_yy/qyjh/getDxqInfo?dxq_id="..dxq_id)
...
--根据上传数量获取前N个学校列表结束
--根据上传数量获取前N个教师列表开始
--[[local tteaids = ssdb:zrrange("qyjh_dxq_tea_uploadcount_"..dxq_id,0,6)
local teaids = {}
local teas = {}
if #tteaids>=2 then
	for i=1,#tteaids,2 do
		table.insert(teaids,tteaids[i])
	end
	local res_person = ngx.location.capture("/dsideal_yy/person/getPersonNameListByIds?ids="..teaids[1])
...
local hdlist
local params = "?page_type=2&hd_type=0&path_id="..dxq_id.."&pageSize=3&pageNumber=1"
