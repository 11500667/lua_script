...
if obj_type == "1" then
	--获取资源数据
	res = mysql_db:query("SELECT SQL_NO_CACHE id FROM t_base_publish_sphinxse WHERE query='"..perfilter.."filter=b_delete,0;filter=obj_type,1;filter=pub_type,3;filter=pub_target,"..path_id..";filter=qyjh_id,"..qyjh_id..";sort=attr_desc:ts;limit="..limit.."'")
	for i=1,#res do
...
elseif obj_type == "3" then
	--获取试卷数据
	res = mysql_db:query("SELECT SQL_NO_CACHE id FROM t_base_publish_paper_sphinxse WHERE query='"..perfilter.."filter=b_delete,0;filter=obj_type,3;filter=pub_type,3;filter=pub_target,"..path_id..";filter=qyjh_id,"..qyjh_id..";sort=attr_desc:ts;limit="..limit.."'")
...
elseif obj_type == "4" then
	--获取备课数据
	res = mysql_db:query("SELECT SQL_NO_CACHE id FROM t_base_publish_sphinxse WHERE query='"..perfilter.."filter=b_delete,0;filter=obj_type,4;filter=pub_type,3;filter=pub_target,"..path_id..";filter=qyjh_id,"..qyjh_id..";sort=attr_desc:ts;limit="..limit.."'")
	
	for i=1,#res do
		local iid = cache:hget("publish_"..res[i]["id"],"obj_info_id")
		local bk_tj_hot = cache:hmget("resource_"..iid,"resource_title","resource_format","resource_page","file_id","thumb_id","preview_status","width","height","for_urlencoder_url","for_iso_url","down_count","scheme_id_int","resource_id_int","create_time","resource_size_int","beike_type")
		--根据版本ID获取该版本属于哪个学科    
...
elseif obj_type == "5" then
	--获取微课数据
	res = mysql_db:query("SELECT SQL_NO_CACHE id FROM t_base_publish_wk_sphinxse WHERE query='"..perfilter.."filter=b_delete,0;filter=obj_type,5;filter=pub_type,3;filter=pub_target,"..path_id..";filter=qyjh_id,"..qyjh_id..";sort=attr_desc:ts;limit="..limit.."'")
