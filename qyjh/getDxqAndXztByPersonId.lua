--[[
根据当前用户ID获取用户所管理的和所属于的大学区、协作体列表
@Author  chenxg
@Date    2015-02-03
--]]
...
for i=2,#dxqlist,2 do
	if cjson.decode(dxqlist[i]).b_delete ~=1 then
		local t = cjson.decode(dxqlist[i])
		
		local xztlist = {}
		local qyjh_id = cjson.decode(dxqlist[i]).qyjh_id
		local dxq_id = cjson.decode(dxqlist[i]).dxq_id
		local limit = 10
		local paras = "?qyjh_id="..qyjh_id.."&dxq_id="..dxq_id.."&limit=10&person_id="..person_id
		local res_xzt = ngx.location.capture("/dsideal_yy/ypt/qyjh/getXztByDxqAndPersonId"..paras)
